      ...
      <div className="card" style={{ padding: "20px" }}>
        ...
        {logs.length === 0 ? (
          ...
          <div style={{ overflowX: "auto", WebkitOverflowScrolling: "touch", width: "100%" }}>
            <table className="table" style={{ width: "100%", minWidth: "650px", textAlign: "left", borderCollapse: "collapse" }}>
              <thead>
                <tr style={{ borderBottom: "1px solid #E5E7EB", color: "var(--text-muted)", fontSize: "13px", whiteSpace: "nowrap" }}>
              ...
              <tbody>
                {logs.map(log => (
                  <tr key={log.log_id} style={{ borderBottom: "1px solid #F3F4F6", fontSize: "14px" }}>
                    <td style={{ padding: "12px", color: "var(--text-muted)", whiteSpace: "nowrap" }}>{new Date(log.timestamp).toLocaleString()}</td>
                    <td style={{ padding: "12px", whiteSpace: "nowrap" }}>{getLevelBadge(log.module, log.action)}</td>
                    <td style={{ padding: "12px", fontWeight: "500", color: "var(--navy)", whiteSpace: "nowrap" }}>{log.user_name}</td>
                    <td style={{ padding: "12px", whiteSpace: "nowrap" }}>{log.module}</td>
                    <td style={{ padding: "12px" }}>{log.action}</td>
                    <td style={{ padding: "12px", color: "var(--text-muted)", minWidth: "180px" }}>{log.details}</td>
